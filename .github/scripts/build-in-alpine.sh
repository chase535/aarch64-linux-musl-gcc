...
install_default_static_specs() {
    local gcc="${MPREFIX}/bin/${TARGET}-gcc"
    local specs_dir
    local specs_file
    local dynamic_specs_file
    local raw_specs

    exec > >(tee "${LOG_DIR}/gcc-default-static-specs.log") 2>&1

    specs_dir="$(dirname "$("${gcc}" -print-libgcc-file-name)")"
    specs_file="${specs_dir}/specs"
    dynamic_specs_file="${specs_dir}/dynamic.specs"
    raw_specs="${specs_file}.raw"
    "${gcc}" -dumpspecs > "${raw_specs}"

    python3 - "${raw_specs}" "${specs_file}" <<'PY'
...
def replace_required(value, old, new, section):
    if old not in value:
        raise SystemExit(f"Unable to patch {section}: missing {old!r}")
    return value.replace(old, new, 1)


default_static = "%{!shared:%{!static:%{!static-pie:-static}}}"
self_spec = sections.get("*self_spec:", "").strip()
sections["*self_spec:"] = f"{self_spec} {default_static}".strip() + "\n"

link = sections["*link:"]
link = replace_required(
    link,
    "%{static:-Bstatic}",
    "%{static:-Bstatic -dynamic-linker /lib/ld-musl-aarch64.so.1}",
    "*link:",
)
sections["*link:"] = link
...
with destination.open("w") as output:
    for name in order:
        output.write(f"{name}\n")
        output.write(sections[name].rstrip("\n") + "\n\n")
PY

    cat > "${dynamic_specs_file}" <<'EOF'
*self_spec:
%{shared:}
...
EOF
    ...
    rm -f "${raw_specs}"
    echo "Installed default-static GCC specs: ${specs_file}"
    echo "Installed dynamic opt-out GCC specs: ${dynamic_specs_file}"
}
...
verify_relocatable_toolchain() {
    ...
    # Default output is static through installed GCC specs. dynamic.specs and
    # -shared keep the normal musl dynamic-linking path available.
    ...
    "${cc}" "${smoke_dir}/hello.c" -lc -o "${smoke_dir}/hello-c-lc"
    "${cc}" -specs=dynamic.specs "${smoke_dir}/hello.c" -o "${smoke_dir}/hello-c-dyn"
    "${cxx}" -specs=dynamic.specs "${smoke_dir}/hello.cpp" -o "${smoke_dir}/hello-cpp-dyn"
    "${cc}" -specs=dynamic.specs -fopenmp "${smoke_dir}/openmp.c" -o "${smoke_dir}/hello-openmp-dyn"
    printf 'int answer(void) { return 42; }\n' > "${smoke_dir}/library.c"
    "${cc}" -shared -fPIC "${smoke_dir}/library.c" -o "${smoke_dir}/libanswer.so"
    ...
    file "${smoke_dir}/hello-c-lc" | grep -q 'statically linked'
    ...
    ! readelf -lW "${smoke_dir}/hello-c-lc" | grep -q 'INTERP'

    file "${smoke_dir}/hello-c-dyn" | grep -q 'dynamically linked'
    file "${smoke_dir}/hello-cpp-dyn" | grep -q 'dynamically linked'
    file "${smoke_dir}/hello-openmp-dyn" | grep -q 'dynamically linked'
    file "${smoke_dir}/libanswer.so" | grep -q 'shared object'
    readelf -lW "${smoke_dir}/hello-c-dyn" | grep -q '/lib/ld-musl-aarch64.so.1'
    readelf -lW "${smoke_dir}/hello-cpp-dyn" | grep -q '/lib/ld-musl-aarch64.so.1'
    readelf -lW "${smoke_dir}/hello-openmp-dyn" | grep -q '/lib/ld-musl-aarch64.so.1'
    readelf -dW "${smoke_dir}/hello-c-dyn" | grep -q 'Shared library: \[libc.so\]'
    readelf -dW "${smoke_dir}/hello-cpp-dyn" | grep -q 'Shared library: \[libstdc++.so.6\]'
    readelf -dW "${smoke_dir}/hello-cpp-dyn" | grep -q 'Shared library: \[libgcc_s.so.1\]'
    readelf -dW "${smoke_dir}/hello-openmp-dyn" | grep -q 'Shared library: \[libgomp.so.1\]'
    readelf -dW "${smoke_dir}/libanswer.so" | grep -q 'Shared library: \[libc.so\]'

    test -e "${relocated_dir}/${TARGET}/sysroot/lib/ld-musl-aarch64.so.1"
    test -e "${relocated_dir}/${TARGET}/sysroot/usr/lib/libc.so"
    test -e "${relocated_dir}/${TARGET}/lib64/libstdc++.so"
    test -e "${relocated_dir}/${TARGET}/lib64/libstdc++.so.6"
    test -e "${relocated_dir}/${TARGET}/lib64/libgcc_s.so.1"
    test -e "${relocated_dir}/${TARGET}/lib64/libgomp.so.1"

    mv "${relocated_dir}" "${MPREFIX}"
}

verify_in_ubuntu_aarch64() {
    local smoke_dir="${GITHUB_WORKSPACE}/smoke-test"
    local test_dir
    test_dir="$(mktemp -d "${GITHUB_WORKSPACE}/arm64-runtime-test.XXXXXX")"

    exec > >(tee "${LOG_DIR}/verify-ubuntu-aarch64.log") 2>&1

    # Stage only the statically linked test binaries.
    ...
    cp "${smoke_dir}/hello-c-lc" "${test_dir}/"

    # Run in an arm64 Ubuntu container through Docker + QEMU binfmt_misc.
    local arm64_image="${ARM64_RUNTIME_IMAGE:-ubuntu:24.04}"
    docker run --rm --platform linux/arm64 \
        ...
        expect_ok_output() {
            ...
            if [ "${output}" != "ok" ]; then
                echo "${label}: unexpected output: ${output}" >&2
                exit 1
            fi
            echo "${label}: OK"
        }
        ...
        expect_ok_output hello-c /test/hello-c
        expect_ok_output hello-cpp /test/hello-cpp
        OMP_NUM_THREADS=2 /test/hello-openmp
        echo "hello-openmp: OK"
        /test/hello-libgcc
        echo "hello-libgcc: OK"

        expect_ok_output hello-c-lc /test/hello-c-lc

        echo "=== All Ubuntu aarch64 tests passed ==="
    '
